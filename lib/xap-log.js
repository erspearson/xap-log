...
(function (xaplog) {
    ...
    if (argv.level) {
        logLevel = 0 + argv.level;
    }
    if (argv.source) {
        try {
            sourceFilter = RegExp(argv.source, 'i');
        }
        catch (e) {
            die(`Invalid source filter '${argv.source}'`);
        }
    }
    if (argv.class) {
        try {
            classFilter = RegExp(argv.class, 'i');
        }
        catch (e) {
            die(`Invalid class filter '${argv.class}'`);
        }
    }
    if (argv.dump) {
        dump = true;
    }
    if (argv.heartbeat && typeof (argv.heartbeat) == 'number') {
        hbInterval = argv.heartbeat;
    }
    ...
    let network = new xap_framework_1.xAP.networkConnection({
        source: {
            vendor: 'xFx',
            device: 'logger',
    ...
    log(1, 'connecting...');
    network.connect();
})(xaplog = exports.xaplog || (exports.xaplog = {}));
//# sourceMappingURL=xap-log.js.map